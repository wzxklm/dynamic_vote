...
特殊情况处理：
- 如果用户输入包含多个值（用"和""与""+"","等连接），只要其中任意一个与候选项等价，即视为匹配
...
额外判断标准：
- 组合值包含关系也算等价：如 "hysteria2和vless" 与 "Hysteria2" 应归为一组（组合值是单值的超集）
- 带修饰词的变体：如 "sni用大学域名" 和 "SNI使用大学域名" 应归为一组

示例输入：
[{"id":"a","value":"Hysteria2","submitCount":5},{"id":"b","value":"hy2","submitCount":2},{"id":"c","value":"VLESS","submitCount":3}]

示例输出：
{"clusters":[{"canonical_id":"a","member_ids":["a","b"]}]}
...
如果没有任何等价组，返回：{"clusters": []}`;
  ...
  const makeRequest = async (): Promise<ClusterResult> => {
    ...
    return { clusters: rawClusters } as ClusterResult;
  };
